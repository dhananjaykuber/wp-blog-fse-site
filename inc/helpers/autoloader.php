...
namespace Starter_FSE_Theme\Inc\Helpers;
...
function autoloader(
	...
	$resource_path  = false;
	$namespace_root = 'Starter_FSE_Theme\\';
	...
	if ( 'inc' === $path[0] ) {
		...
		$resource_path = sprintf( '%s/inc/%s/%s.php', untrailingslashit( STARTER_FSE_THEME_TEMP_DIR ), $directory, $file_name );
	...
	if ( 'tests' === $path[0] ) {
		...
		$resource_path = sprintf( '%s/tests/php/%s.php', untrailingslashit( STARTER_FSE_THEME_TEMP_DIR ), $file_name );
...
spl_autoload_register( '\Starter_FSE_Theme\Inc\Helpers\autoloader' );
